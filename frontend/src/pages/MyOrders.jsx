...
function MyOrders() {
    ...
    return (
        <div className="mt-20 pb-20 max-w-5xl mx-auto px-4">
            <div className="mb-10">
                <h1 className="text-3xl font-bold text-gray-800 uppercase tracking-wide mb-2">My Orders</h1>
                    ...
                    <div key={idx} className="bg-white shadow-lg rounded-xl mb-10 p-6">
                        <div className="flex flex-col md:flex-row md:justify-between md:items-center mb-6 gap-2">
                            ...
                            <div>
                                ...
                                <span className="ml-2 text-primary font-bold">${order.amount}</span>
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <span className="ml-2 text-gray-500">{new Date(order.createdAt).toLocaleDateString()}</span>
                            </div>
                        </div>
                        <div className="divide-y divide-gray-200">
                            {order.items.map((item, i) => (
                                <div key={i} className="flex flex-col md:flex-row md:items-center py-4 gap-6">
                                    ...
                                    <div className="flex-1 flex flex-col md:flex-row md:justify-end md:items-center gap-4">
                                        <div>
                                            <p className="text-gray-600">Quantity: <span className="font-medium">{item.quantity || 1}</span></p>
                                        </div>
                                        <div>
                                            <p className="text-primary font-semibold">Amount: ${item.product.offerPrice * (item.quantity || 1)}</p>
                                        </div>
                                    </div>
                                </div>
                            ))}
